<template>
    <div class="central">
            ...
            <div>
                <Button icon="mdi mdi-plus" class="h-btn h-btn-s" @click="add(datasc)">添加</Button>
                <Button icon="mdi mdi-delete" class="h-btn h-btn-s" text-color="red" @click="remove()">删除</Button>
            </div>
            <Table :datas="datasc" stripe checkbox ref="tablec" align="center" @trclick="detaile()" >
                <TableItem title="序号">
                    <template slot-scope="{index}">{{index}}</template>
                </TableItem>
                <TableItem title="路由" prop="name"></TableItem>
                <TableItem title="操作">
